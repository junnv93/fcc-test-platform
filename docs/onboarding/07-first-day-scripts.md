...
그 방식은 하위 프로세스에 `GIT_CONFIG_PARAMETERS` 를 넘겨서, 격리를 전제로 하는
검사 하나를 깨뜨립니다(실측 2026-09-07). **설정으로 두고 그냥 `git push` 하십시오.**
...
| `git -c core.hooksPath=…` 로 훅 임시로 켜기 | 하위 프로세스에 설정이 새어 검사 하나를 깨뜨립니다 (실측 2026-09-07) |
