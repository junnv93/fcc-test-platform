...
⚠️ **이 비대칭이 «의도»인지 «누락»인지 이 문서는 모릅니다.** 의도일 수 있는 근거가
하나 있습니다 — contracts 는 **태그 레인**이고, platform 의 핀이 커널 태그를 가리키므로
CI 할당량 사고 등으로 검사가 멈추면 **공급 사슬 전체가 멈춥니다.** `enforce_admins: false`
는 그때 운영자가 손으로 푸는 비상구입니다. 하지만 **그 근거가 어디에도 적혀 있지
않으므로, 오늘은 「우회할 수 있다」만 참입니다.** 운영자 판정이 필요합니다.
