...
| **`enforce_admins`** | ✅ **`true`** | ✅ **`true`** | ✅ (13:4x 부터) |

### ✅ 비대칭이 «해소됐습니다» — 두 레인 모두 `enforce_admins: true`

**두 레인 다 관리자가 보호를 우회하지 못합니다** (2026-09-07 13:4x 부터).
즉 **「red 면 서버가 막는다」가 두 레인에서 참**입니다.

<!-- 정정 이력 — 지우지 마세요. 이 절은 하루에 «세 번» 바뀌었습니다. -->
> 🔄 **정정 (13:4x).** 이 절의 앞 판은 *「### 🔴 남은 비대칭 하나 — `enforce_admins` …
> **`fcc-test-contracts` 에서는 관리자가 보호를 우회해 `main` 에 직접 push 할 수
> 있습니다.** platform 에서는 못 합니다.」* 였습니다.
>
> **그 비대칭은 «의도»였고, 그 의도의 전제가 오후에 죽었습니다.** 아래 인용이
> 그 옛 근거입니다 — **지우지 않고 남깁니다.** 되돌려야 하는 날 다시 읽어야 하기
> 때문입니다(되돌리는 조건은 `fcc-test-contracts/CLAUDE.md` §`enforce_admins`).

**옛 근거 (오전 판, 이제는 전제가 해소됨)** — `fcc-test-contracts/CLAUDE.md`
§`enforce_admins` 절:
...
**그 전제가 2026-09-07 오후에 해소됐습니다.** 두 레포가 `public` 이 되어 표준 러너를
무과금으로 받고, 최근 런이 전부 `success` 이며 `runner_name` 이 채워집니다.
전제가 죽으면 그 위의 결론도 죽습니다 — 그래서 `true` 로 올렸습니다.

⚠️ **옛 판이 적었던 「평시의 대가」도 함께 사라졌습니다.** 그 판은 이렇게 적었습니다 —
*「이 레포의 협업자는 1명이고 그 1명이 `admin` 이므로 «사실상 아무나, 언제든» 우회할
수 있다. 이 레포에서 보호가 실제로 하는 일은 「PR 을 거치게 하는 것」 하나이고,
`lane-check` 은 그 PR 에 붙는 «조언»이다. **「red 면 서버가 막는다」는 이 레인에서
거짓이다.**」* — **오늘 그 문장들은 전부 거짓입니다.**

⚠️ **되돌리는 조건**: 할당량이 다시 차서 이 레인의 `lane-check` 이 **러너를 못 받으면**
이 값이 공급 사슬을 멈춥니다. 그날 볼 것은 `conclusion` 이 아니라 **`runner_name`**
입니다.
...
> `lane-check` 가 required 입니다.
>
> 🔄 **그리고 이 정정문 자신도 몇 시간 만에 낡았습니다.** 이 자리의 12:08 판은
> *「다만 **완전히 해소된 것은 아닙니다** — 위 표의 `enforce_admins: false` 행이 남은
> 구멍이고 … 옛 판의 「누구나 직접 push」는 이제 「**관리자만** 직접 push」입니다」*
> 였습니다. **13:4x 에 `enforce_admins` 가 `true` 가 되어 그 구멍도 닫혔습니다** —
> 이제 「관리자만」도 아닙니다.
...
⑤ fcc-test-contracts 의 enforce_admins 판정                  ✅ 완료 — true (13:4x, 운영자 승인)
```

**오늘 서 있는 자리는 ③ 입니다.** ②까지 끝났고 ③이 사람의 클릭을 기다립니다.

### 🔴 ④ 를 판단할 때 «협업자 수»만 보지 마십시오 — PR 속도를 먼저 재십시오

`required_approving_review_count: 1` 은 「협업자가 둘 이상인가」로 결정할 문제가
아닙니다. **하루에 몇 건이 그 게이트를 통과해야 하는가**가 실제 비용입니다.

실측 (2026-09-07, `gh pr list --state merged`):

| 날짜 | `fcc-test-platform` 머지 |
|---|---|
| 2026-09-07 | **24건** |
| 2026-09-06 | **49건** |
| 2026-09-05 | 25건 |
...
```bash
gh pr list --repo junnv93/fcc-test-platform --state merged --limit 100   --json mergedAt --jq '[.[].mergedAt[0:10]]|group_by(.)|map({d:.[0],n:length})|sort_by(.d)|reverse|.[0:7][]'
```

⚠️ **그리고 그 PR 은 거의 전부 에이전트가 냅니다** — 전부 `junnv93` 계정입니다.
GitHub 은 자기 PR 자기 승인을 금지하므로, 승인수를 `1` 로 켜는 순간
**하루 24~49건이 «사람 둘»의 클릭을 기다립니다.**

**이것은 `CODEOWNERS` 가 이 규모에서 게이트가 될 수 없는 것과 같은 형태입니다**
(`02-how-we-collaborate.md` §3) — 끄면 아무것도 안 막고, 켜면 아무도 못 고칩니다.
그리고 그 압력은 **읽지 않은 승인**을 만듭니다. 이 레포는 그것을 이미 이름으로
기록했습니다 — **「항상 승인되는 승인은 승인이 아니라 서명」**
(`intent/README.md` §승인 모델, 2026-09-07 개정).

> ### 🧭 ④ 를 하기 전에 스스로 묻는 질문
> **「이 게이트를 통과해야 하는 것이 하루 몇 건인가, 그리고 누를 사람이 몇 명인가?」**
> 그 비가 감당 못 할 값이면, 그 게이트는 **켜지는 순간 우회되거나 형식이 됩니다.**
> 대안은 「전부 승인」이 아니라 **판단이 필요한 소수를 나머지와 다르게 «생기게»
> 만드는 것**입니다 — 그것이 `intent/README.md` 의 **방아쇠 다섯**입니다.
