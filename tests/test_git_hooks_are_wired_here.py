...
class TestTheProbeDoesNotDisarmTheRepositoryItChecks(unittest.TestCase):
    ...
    # ⚠️ 이 클래스의 **모든** git 호출이 환경을 격리한다 — 봉인 자신이 같은 결함을
    #    갖지 않기 위해서다. 실측 2026-08-31: 초판은 `_victim` 만 격리를 빠뜨렸고,
    #    훅 안에서는 부모의 `GIT_DIR` 이 **이미** 서 있으므로 피해자를 만들려던
    #    `git init`/`git commit` 이 **진짜 저장소**를 향했다(`core.bare` 가 뒤집혔고,
    #    커밋은 *"nothing to commit, working tree clean"* 으로 실패했다 — 그 트리가
    #    victim 이 아니라 개발자의 워크트리였기 때문이다).
    #    ⚠️ **격리 단위 실행으로는 잡히지 않는다** — 그 문맥엔 주변 `GIT_DIR` 이 없어
    #    같은 코드가 정상 동작한다. 훅을 통한 실제 push 가 잡았다.
    ...
    def _victim(self, root: Path) -> Path:
        ...
        repo = root / 'victim'
        init = self._git('init', '-q', str(repo))
