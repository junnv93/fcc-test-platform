...
🔴 **이 구멍은 «관측됐습니다» — PR #137 이 그 사례입니다** (2026-09-07 실측).
...
즉 **#137 은 두 PR 만큼 낡은 초록 위에서 머지됐습니다.**
다만 **해는 없었습니다** — #137 이 만진 30파일과 그 사이 들어온 4파일의
**교집합이 0** 입니다(PR API 의 파일 목록으로 실측).

> 정확한 문장: **「낡은 초록으로 머지됐다」의 사례이지 「낡은 초록 때문에
> 깨졌다」의 사례가 아니다.**

### ⚠️ 이 기제를 «영원히 못 보는» 측정 방법이 있습니다

이 문서의 앞 판은 *"둘 다 그때의 최신 main 과 합쳐 검사했으므로 반례가 아니다"*
라고 적었고 **그것이 틀렸습니다.** 그 판정은 이렇게 나왔습니다:

```
gh run view --log | grep "HEAD is now at"      ← 마지막 «실행» 하나를 본다
```

GitHub 은 실행할 때마다 머지 ref 를 **다시 계산**하므로, 그 명령은 **언제나**
「그때의 최신과 합쳤다」로 답합니다. 실측(같은 PR 의 실행 전수):

```
21:50  Merge 89bf175 into 85c43a4
21:54  Merge 44d40af into dc9fdf9   ← base 가 바뀌었다
23:02  Merge bc97817 into ae7d7d4
```

물어야 할 것은 **검사한 base 와 «머지 커밋의 첫 부모»의 차이**입니다:

```sh
git log -1 --format=%P <머지커밋> | cut -d' ' -f1   # 머지 시점의 main
git log --oneline <검사한 base>..<그 값>            # 그 사이 들어온 것
```

> 🔑 **도구가 답한 것이 내가 물은 것이 아니었습니다.** 이 세션이 하루에 다섯 번째로
> 밟은 형태이고, 형제 세션 `fcc-delivery-final-4a` 가 잡았습니다.
