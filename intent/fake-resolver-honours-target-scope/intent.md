...
1. **대역이 target 을 «모르는» 등록만 갖고 있을 때, target 을 물으면 어떻게 답해야
   ...
   → 답(운영자 — 2026-09-07): **(c) 를 고른다.** 「선언하지 않은 무관심」과 「선언한
      스코프를 무시함」은 다른 명제다. `{11:'AAA'}` 는 「스코프 무관하게 11은 AAA」라는
      caller 의 «선언»이고 대역이 그것을 존중하는 것은 결함이 아니다. 결함은 caller 가
      스코프를 선언했는데 대역이 그것을 넘어 미끄러지는 경우다 — 그때만 loud 다.
      (b) 를 버린 이유: 오늘 초록인 8건은 target 을 신경 쓰지 않는다고 «정당하게»
      선언한 시험이다. 게이트가 오탐을 내면 사람들은 우회를 배우고, 그날부터 그
      게이트는 0층이다 — 이 레포의 intent/README.md 가 이미 적어 둔 문장이다.
...
2. **오늘의 픽스처를 target 있는 것으로 바꿔야 하나, 아니면 target 있는 픽스처를
   ...
   초안 의견은 **더한다**.
   → 답(세션 fcc-delivery-final-c1 — 개발 담당, 2026-09-07): **더한다.** target 이
      빈 세계도 생산에 존재한다 — `measurement_target_key` 는 model·sample 중 하나만
      없어도 `''` 를 내고(실측 `('M1', None) → ''`), `session_uuid_name` 은 그 경우
      legacy 식별자 공간을 «일부러» 보존한다(`f'{provider}:{local_id}'`). 픽스처를
      바꾸면 그 세계를 검증하던 8건이 사라진다. 두 세계가 다 남아야 한다.
...
3. **`register_session` 의 호출부가 0건인데 그것을 넓히는 것이 옳은가?** 「아무도 안
   ...
   → 답(운영자 — 2026-09-07): **넓힌다.** 호출부 0건은 「고칠 필요 없다」가 아니라
      「지뢰가 아직 안 밟혔다」는 뜻이다. reader 가 3-튜플을 읽고 있으므로 writer 가
      그것을 못 만드는 것 자체가 다음 사람이 밟을 자리다. 「지운다」는 공개 표면 축소라
      이 판의 범위를 넘고 별도 intent 가 필요하다.
...
4. **chamber 정규화 갈라짐을 이 intent 가 이름으로만 남기고 넘기는 것이 맞나?**
   ...
   고정하라」에 정면으로 걸린다.
   → 답(세션 fcc-delivery-final-c1 — 개발 담당, 2026-09-07): **넘긴다.** 「두 축이
      함께 움직이면 하나를 고정하라」에 정면으로 걸린다. 이 판이 target 축을 고치고도
      실물·대역이 여전히 갈리면, 그 갈라짐의 원인이 chamber 정규화라는 것이 «남은 하나»
      이므로 오히려 더 선명하게 드러난다. non_scope 에 이름으로 적어 뒀다.
