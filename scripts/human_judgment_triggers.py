...
이 레인에는 **승인자가 없다.** 개발 담당자가 각자 확정하고 진행한다
(2026-09-07 개정 2차 — 협업자가 1명이라 「작성자 제외 동료」가 존재하지 않았다).
규칙 본문 SSOT 는 `intent/README.md`.

그래서 이 파일이 «유일한» 사람 판단 요구 지점이다 — 승인 버튼이 사라진 자리를
메우는 것이 아래 방아쇠들이고, 이것들은 버튼이 아니라 **문장**을 요구하므로
혼자 일해도 공허해지지 않는다.
