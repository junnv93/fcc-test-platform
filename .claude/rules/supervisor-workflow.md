...
## claim 축만으로 소유를 판정하지 마라 — 하루에 세 번 중복했다 (2026-09-06)

위 **Operating Rules** 첫 줄은 *"두 브랜치의 `scope` 가 겹치면 **claim 파일**이 순서를
적어야 한다"* 고 말한다. ⚠️ **그 규칙 자체가 claim 파일의 존재를 전제한다.**
`ls .claude/work-claims/` 가 비어 있을 때, 그것은 「충돌이 없다」가 아니라
**「이 축으로는 답할 수 없다」**일 수 있다.

### 「claim 0건」은 세 가지 다른 사실을 가리킨다

| 본 것 | 실제 | 무엇이 답을 주나 |
|---|---|---|
| claim 0건 | 기제가 **이 레포에 없다** — 다른 레포에 있다 | 그 레포의 `.claude/work-claims/` |
| claim 0건 | 있는데 **커밋 안 됐다** — 남의 워크트리에 untracked | `git worktree list` |
| claim 0건 | **기제는 있는데 관행이 없어 비었다** | 훅 경고 · 소스 |

셋째가 가장 나쁘다. 이 레포에는 `scripts/work_claim_branch_guard.py` 가 있고
`.claude/work-claims/` 를 보는데 **아무도 쓰지 않아 0건**이다. 도구가 있으니 읽는
사람은 claim 을 권위 있는 답으로 여기는데 `ls` 는 정직하게 비었다고 답한다.
게다가 그 가드는 fail-open 이라 바인딩 없이도 커밋이 통과한다(경고만 낸다).

### 실제 사고 (2026-09-06) — 두 세션이 하루에 세 번 같은 일을 시작했다

```
① 모노레포 v0.1.9 핀      claim 을 «공유 체크아웃»에서 조회 → 0건 → 착수
                          실제로는 다른 세션의 워크트리에 untracked claim 이 있었다
② PR #82 리베이스·검증    양쪽이 각자 워크트리에서 같은 리베이스를 완주
③ 교육자료 §06 수정       「내가 잡겠다」 메시지가 «교차»했다
```

셋 다 **작업을 시작한 뒤에** 알았다. ①은 편집 5파일을 되돌렸고, ②는 두 세션이 같은
리베이스를 각자 완주했다.

### 그래서 무엇을 보라

1. **`git worktree list` 를 «건드릴 레포에서» 보라.** 나는 platform 에서 세 축을 다
   보고 모노레포에서는 claim 축만 봤다. 핀은 모노레포에 있었다.
2. **커밋 0개인 브랜치도 진행 중일 수 있다.** `git log origin/main..<브랜치>` 가 비어도
   일은 워크트리의 «미커밋 상태»에 있다 — `git -C <워크트리> status --short` 까지 보라.
3. ⚠️ **워크트리 mtime 은 낡는다.** mtime 이 21:27 인 워크트리의 세션이 07:59 에
   살아 있었다. 살아 있는지를 말하는 것은 그 세션의 **전사 파일**이다:

   ```bash
   ls -lt ~/.claude/projects/<프로젝트>/*.jsonl | head
   ```

   워크트리 경로에 세션 id 가 박혀 있으므로 둘을 이어 볼 수 있다.

### 그래도 남는 창 — 규칙은 「먼저 보라」가 아니다

위 셋을 다 해도 «동시에 시작하는» 경우는 못 막는다(③이 그랬다 — 두 메시지가 교차했다).
그러니 이 절의 규칙은 **탐지가 아니라 해소**다:

> 겹친 것을 알면 **즉시 알리고**, 이미 사용자 승인을 받은 쪽이 잡는다.
> 나머지는 그 파일을 열지 않았음을 확인해 주고 **겹치지 않는 축으로 옮긴다.**

③에서 실제로 그렇게 풀었다. 그리고 그렇게 갈라진 두 세션이 서로의 결과를 **다시 재서**
각각 결함을 하나씩 찾았다 — 중복은 낭비지만, 갈라진 뒤의 교차 검증은 낭비가 아니다.
