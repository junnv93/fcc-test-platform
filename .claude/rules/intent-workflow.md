...
그 다음 `spec.md`(확정), 그 다음 `plan.md`(확정), 그 다음 코드. **셋 다 개발 담당자 본인이 확정한다.**

**초안은 전부 에이전트가 쓰고 사람은 «확정»만 한다** — `Status: accepted` 를 적는 것.
사람이 초안을 쓰면 속도의 이득이 사라지고, 사람이 확정을 안 하면 책임이 사라진다.

## 승인자는 없다 — 개발 담당자가 각자 확정한다

🔄 **2026-09-07 개정 (2차).** 「동료 1인(작성자 제외)」을 폐기했다 — 협업자가 1명이라
그 사람이 **존재하지 않았다**. 남의 결재를 기다리지 마라. 에이전트는 사용자에게
「동료 승인을 받아야 한다」고 말하지 마라.

⚠️ 승인이 사라졌다고 사람 판단이 사라진 것은 아니다. 방아쇠 다섯이 걸릴 때
사람의 «기록된» 결정이 여전히 요구되고, 이것들은 버튼이 아니라 **문장**을 요구한다:
