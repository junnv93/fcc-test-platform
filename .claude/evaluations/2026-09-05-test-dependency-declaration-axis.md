...
> 🔴 **회수 기록 (2026-09-07).** 이 평가서는 브랜치
> `fix/test-dependency-declaration-axis-20260905` 에만 있었고 `main` 에 착지한 적이
> 없다. 정리 중에 발견해 **평가서만** 회수했다.
>
> ⚠️ **아래 §What 3 이 말하는 `tests/test_test_dependency_declaration_axis.py` 는
> 회수하지 않았다 — 그 파일은 이 저장소에 «없다».** 같은 명제를 `main` 의
> `tests/test_supply_closure_axis.py` 가 **더 넓게** 지키기 때문이다:
>
> | | 이 평가서의 봉인 | `main` 의 `test_supply_closure_axis` |
> |---|---|---|
> | 미선언 3rd-party import | ✅ | ✅ (`test_every_unguarded_third_party_import_resolves_to_a_declared_distribution`) |
> | 비-공허성 | ✅ | ✅ |
> | 해소 불가 이름의 원장 | ❌ | ✅ (`test_unresolvable_imports_match_the_recorded_ledger_exactly`) |
> | 배포판 목록 자체 | ❌ | ✅ |
> | 휠이 자원을 싣는가 | ❌ | ✅ (`TestEveryPackageResourceShipsInTheWheel`) |
> | 판정기의 소유 | 이 파일 안 | 계약 레인의 `fcc_test_contracts.common.supply_closure` (사본 0) |
>
> 즉 봉인을 되살리면 **같은 축의 두 번째 사본**이 되고, 이 저장소가 여러 번 값을 치른
> 형태다. 되살린 것은 **실측**이다 — 아래 §Why 의 표(`21 skipped` · `166 passed`)가
> `main` 어디에도 없었다.
>
> ⚠️ **§What 1·2 는 이미 착지해 있다**(확인 2026-09-07): `pyproject.toml` 에
> `PyYAML>=6.0` 이 선언돼 있고, 걷어냈다는 가드 넷의 자리에 그 사실이 주석으로 남아
> 있다 — `tests/test_central_docker_compose.py:27` · `tests/test_auth_mode_pairing.py:27`.
> 그리고 `test_central_docker_compose.py:41` 의 `import yaml` 은 최상위·무가드다.
>
> ⚠️ 아래 §후속 두 항목은 **재확인하지 않았다.** 2026-09-05 시점의 기록으로 읽어라.
