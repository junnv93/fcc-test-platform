...
## 🔁 그리고 «내가 사실을 바꾸자» 내 문서가 다시 낡았다 — 같은 날 네 번째

운영자 승인으로 `fcc-test-contracts` 의 `enforce_admins` 를 `false → true` 로 올렸다
(13:4x). 그 순간 **이 웨이브가 몇 시간 전에 쓴 서술 열여덟 곳이 거짓이 됐다** —
두 레인에 걸쳐서.

| 레포 | 자리 |
|---|---|
| `fcc-test-contracts` | `CLAUDE.md` §`enforce_admins` 절(근거 SSOT) · `CODEOWNERS` · `README.md` |
| `fcc-test-platform` | `CLAUDE.md` 거짓서술표 · `02` §4 표 · `05` §3(4곳) · `06` §4(3곳) · `20` · 교육자료 2종(6곳) |

그리고 같은 날 contracts PR #57 이 `.claude/rules/` 를 0 → 3 으로 만들면서
**「0개」라 적은 자리 셋이 또 낡았다.**

### 이번 사례가 앞의 열하나와 다른 점

앞의 열하나는 **남이(또는 시간이) 바꾼 사실**을 내가 뒤늦게 발견한 것이다.
이번은 **내가 바꾼 사실**이다. 즉 「몰랐다」가 성립하지 않는다.

> ### 🧭 이 사례가 더하는 질문
> **「내가 방금 바꾼 이 값을, 어느 문서가 «값으로» 적고 있는가?」**
> 설정을 바꾸는 명령과 그 사실을 적은 문서를 **같은 커밋 묶음**으로 다루십시오.
> 값이 바뀌는 시점은 «알 수 있는 시점»이므로, 이 유형은 **원리적으로 100% 막을 수
> 있는 유일한 갈라짐**입니다.

### 그리고 뒤집힌 방식이 특이하다 — 근거가 아니라 «전제»가 죽었다

옛 논거(「태그 레인이 멈추면 공급 사슬이 멈춘다」)는 **틀리지 않았다.**
그 논거가 서 있던 전제 — *「러너 할당량 문제의 원인이 아직 해소되지 않았다」* —
가 해소됐다. **결정을 뒤집기 전에 근거의 진위를 재라**는 규율의 변형이다:
근거가 참인데도 결정이 뒤집힐 수 있고, 그때 봐야 하는 것은 **근거의 전제**다.

그래서 `contracts/CLAUDE.md` 에 **되돌리는 조건**을 실행 가능한 형태로 적었다 —
`runner_name` 이 비면 러너 부재이고, 그날 운영자 승인 아래 `DELETE` 로 되돌린다.

## 후속 — 남은 것과 그 이유

| 항목 | 상태 |
|---|---|
| `fcc-test-contracts` 의 `.claude/rules/` | ✅ **해소 — 3개**(contracts PR #57): 배포판 버전 · 경로 해소기 · 선언과 소비 레인 |
| `enforce_admins` 비대칭 (contracts `false`) | ✅ **해소 — `true`**(13:4x, 운영자 승인). 전제(러너 할당량 미해소)가 죽어 결정이 뒤집혔다. 되돌리는 조건은 `contracts/CLAUDE.md` §`enforce_admins` |
