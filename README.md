...
### 1. 무엇을 고칠 수 있는지는 **파일마다 다릅니다**

`.extraction-layout.json` 에 등재된 **921개 경로**는 배송이 관리합니다 — 고치면 배송이
그 파일을 이름으로 대며 거부합니다. 그 밖의 것(이 README · `docs/architecture/**` ·
새로 더하는 파일)은 **여기서 자유롭게 작성·수정**합니다. 판정법은 위 배너에 있습니다.
...
이 레인은 정식 분리되어 여기서 개발합니다. 다만 **배송이 관리하는 921개 경로**는
예외이고, 그 경계가 이 절의 주제입니다.
