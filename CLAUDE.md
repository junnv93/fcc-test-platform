...
`intent/<slug>/intent.md` 를 만들고 **사람이 확정(`Status: accepted` + PR 머지)** 한다.

* 세 파일: `intent.md` → `spec.md` → `plan.md`
* **승인자는 없다.** 개발 담당자가 각자 확정하고 진행한다(2026-09-07 개정 2차).
  남의 결재를 기다리지 않는다. 사람의 결정이 «기록»으로 요구되는 것은 기계가
  판정하는 방아쇠 다섯뿐이다:
  되돌릴 수 없는 경로(`CODEOWNERS`) · 기준선에 이름 추가 · 답 없는 열린 질문 ·
  `plan.md` 표에 없는 파일 · 선언 대비 과대 변경.
  ⚠️ 방아쇠는 **알림이 아니라 red** 다 — 아티팩트에 이름 붙은 결정이 적히기 전까지
  검사가 초록이 되지 않는다.
* **초안은 전부 에이전트가 쓰고, 사람은 확정만 한다** — `Status:` 는 사람이 적는다.
* 규칙 본문 SSOT → **`intent/README.md`**. 서식 → `intent/_templates/`.
* ⚠️ **적용 안 함**: 오탈자 · 명백한 버그 한 줄 · 이미 확정된 spec 안의 후속 수정.
...
### 6. 세션이 만든 교육자료는 «저장소에» 남긴다

시험원·협업자 교육자료를 만들었으면 아티팩트로 게시하는 것으로 **끝내지 않는다.**
파일을 **커밋한다**:

```
docs/education/YYYY-MM-DD-<주제>-시험원-교육자료.html      ← 시험원 대상
docs/education/YYYY-MM-DD-<주제>-협업자-교육자료.html      ← 개발/협업자 대상
```

* 파일 형태는 아티팩트와 **같다** — `<title>` 로 시작하는 자족 HTML 이고
  `<!doctype>`/`<head>`/`<body>` 를 쓰지 않는다. 외부 의존은 Google Fonts 하나뿐.
* `<주제>` 는 그날 배운 것의 짧은 이름이다(「초록의 뜻」·「조용한 결함」·
  「선언보다 넓은 계약」). 작업 제목이 아니라 **다음 사람이 목록에서 고를 이름**이다.
* ⚠️ **아티팩트 URL 은 그 세션의 것이다.** 게시만 하고 끝내면 다음 사람이
  저장소에서 그것을 찾지 못하고, 같은 자료가 다시 만들어진다.

⚠️ 이 항목은 2026-09-07 에 **실제로 누락돼서** 생겼다. 그날 이미 14건이
`docs/education/` 에 커밋돼 있었는데 이 파일 어디에도 그 이름이 없었고, 세션은
아티팩트만 게시하고 끝냈다. **관례가 저장소에 있어도 적혀 있지 않으면 규칙이 아니다.**
